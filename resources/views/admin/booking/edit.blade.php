<form action="{{ route('admin.booking.update', $booking->id) }}" class="modal-form" method="POST">
    @csrf
    @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="id_user" id="id_user">
                ...
                @foreach ($clients as $item)
                    <option value="{{ $item->id }}" {{ $booking->id_user == $item->id ? 'selected' : '' }}>
                        {{ $item->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="id_paket" id="id_paket">
                ...
                @foreach ($pakets as $item)
                    <option value="{{ $item->id }}" {{ $booking->id_paket == $item->id ? 'selected' : '' }}>
                        {{ $item->nama }}
                    </option>
                @endforeach
            </select>
